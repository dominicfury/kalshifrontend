...
- **suggested_stake_pct_of_bankroll** (decimal fraction; e.g. 0.0061 = 0.61% of bankroll): quarter-Kelly stake as a fraction of bankroll. Bankroll-agnostic — multiply by the user's bankroll size to get the dollar stake. Null when edge < 2% (no stake suggested — these rows are track-only). Quote as a percentage when explaining (e.g. "stake 0.61% of bankroll, so $0.61 on a $100 bankroll, $6.10 on a $1,000 bankroll"). Reasoning: full Kelly assumes the edge estimate is exactly right; ¼ Kelly absorbs the safety factor needed when the consensus model occasionally overstates edge.
